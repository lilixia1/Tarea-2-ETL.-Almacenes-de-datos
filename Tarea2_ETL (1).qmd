...
Para poblar la dimensión Diagnosis se diseñó un flujo ETL que permite transformar diagnósticos complejos en una estructura jerárquica acorde con el modelo en copo de nieve.

1. Extracción
Mediante la consulta SQL proporcionada se obtiene la información desde la tabla Diagnosis. En esta fase se descompone el campo diagnosisString, que contiene rutas diagnósticas separadas por |.
La consulta genera cuatro niveles jerárquicos (diagnosisN1–N4), además de limpiar los campos diagnosisPriority e ICD9Code.
...
2. Conversión de datos
Se aplica un componente Data Conversion para transformar valores Unicode (DT_WSTR) en cadenas no Unicode (DT_STR), compatibles con las tablas dimensionales del DW.

3. Lookups
- Prioridad: relaciona cada diagnóstico con la tabla Prioridad, recuperando la clave sustituta asociada.

- Clasificación: enlaza el diagnóstico con la dimensión Clasificación mediante el valor ICD9.

4. Carga final
Los datos ya transformados y normalizados se insertan en la tabla Diagnosis del Data Warehouse.
...
La dimensión Hospital no requiere cálculos ni transformaciones complejas.

Proceso realizado

1. Extracción directa
Se toma la tabla Hospital completa desde la base de datos transaccional.


2. Conversión de datos
Se normalizan tipos de datos y se convierten cadenas Unicode.


3. Lookups
- Región: asigna la clave sustituta de la dimensión Region.

- Tamaño (número de camas): se vincula con la dimensión TamanoHospital.


4. Carga final
Se inserta cada hospital en la dimensión correspondiente del DW.

![Carga Hospital](imagenes/Carga%20Hospital.png)

### 4. **Carga del Hecho: Ingreso UCI**
El hecho IngresoUCI integra múltiples fuentes y cálculos porque resume el episodio clínico del paciente en la UCI.

Proceso realizado

1. Extracción y preparación de datos
Se calculan varias métricas relacionadas con:

- Apache Final. Selecciona el registro APACHE más reciente para cada estancia.
Convierte valores inválidos (-1) en NULL.
Mantiene apacheScore y mortalidadProbabilidad.

- Ventilacion. Se extrae la duración total de ventilación, número de episodios ventilatorios y un indicador binario de si el paciente fue ventilado.

- Respiración. Obtiene frecuencia respiratoria máxima y mínima desde vitalPeriodic.


2. Cálculos clave
Desde la tabla patient se derivan:

- duracionAdmision = desplazamiento desde ingreso hospitalario.
- duracionUCI = offset de alta de UCI.
- estadoAlta = condición al alta de la unidad.


3. Union de datos
Se combinan los resultados mediante LEFT JOIN, garantizando que todos los ingresos aparezcan aunque falten datos en APACHE o ventilación.
...
4. Carga final
El resultado se envía directamente al destino ADO NET, alimentando la tabla de hechos IngresoUCI.
